...
data1_class1 = generate_uniform_circle_data(1, [-1.1, 0], 50)
data1_class2 = generate_uniform_circle_data(1, [1.1, 0], 50)
...
data2_class1 = generate_uniform_circle_data(1, [-1.1, 0], 50)
data2_class2 = generate_uniform_circle_data(3, [3.3, 0], 50)  # Corrected radius difference
...
data3_class1 = generate_semicircle_data_fixed_range(1, [0, 0], 50, [0, np.pi])
data3_class2 = generate_semicircle_data_fixed_range(1, [1, 0], 50, [np.pi, 2 * np.pi])  # Shifted by radius, angle range adjusted
...
target_dir = 'data/'
df1.to_csv(target_dir + 'equal_radius_circles_100.csv', index=False)
df2.to_csv(target_dir + 'different_radius_circles_100.csv', index=False)
df3.to_csv(target_dir + 'semicircles_with_noise_100.csv', index=False)
